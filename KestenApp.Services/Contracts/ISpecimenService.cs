namespace KestenApp.Services.Contracts
{
    ...
    public interface ISpecimenService
    {
        ...
        Task<SpecimenListModel> AllSpecimensAsync(string userId, SortingType sorting = SortingType.DateCreated, int currentPage = 1, int countPerPage = int.MaxValue, bool isPublished = true);
        Task ArchiveByIdAsync(Guid id, bool restore = false);
        Task ArchiveByIdsAsync(IEnumerable<string> ids);
        Task<SpecimenDetailsModel> GetDetailsViewByIdAsync(Guid id, string userId);
